<?php
	...
	if( !class_exists( 'gardener_widget_post_meta' ) ){
		class gardener_widget_post_meta extends WP_Widget
		{
		    ...
		    {
		        parent::__construct( 'gardener_widget_post_meta', __( 'Post Meta', 'gardener' ) . ' [' . tempo_core::theme( 'Name' ) . ']', array(
		            'classname'     => 'tempo_widget_post_meta',
		            'description'   => __( 'This widget can be used only for single post templates', 'gardener' )
		        ));
		    }
		    ...
		    function widget( $args, $instance )
		    {
		        ...
		        $instance = wp_parse_args( (array)$instance, array(
		            'title' => __( 'Post Meta Details', 'gardener' )
		        ));
		        ...
		        // author link
		        echo '<li><a href="' . esc_url( get_author_posts_url( $post -> post_author ) ) . '" title="' . sprintf( __( 'Written by %s', 'gardener' ), esc_attr( $name ) ) . '"><i class="tempo-icon-user-5"></i>' . esc_html( $name ) . '</a></li>';
		        ...
		        if( $post -> comment_status == 'open' ) {
		            ...
		            echo sprintf( _nx( '%s Comment', '%s Comments', absint( $nr ), '...', 'gardener' ), number_format_i18n( absint( $nr ) ) );
		            echo '</a></li>';
		        }
		        ...
		        if( function_exists( 'stats_get_csv' ) ) {
		            ...
		            echo '<li><i class="tempo-icon-eye-2"></i> ' . sprintf( _n( '%s view', '%s views', absint( $views ), 'gardener' ), number_format_i18n( absint( $views ) ) ) . '</li>';
		        }
			...
			function form( $instance )
 		    {
 		        ...
 		        echo '<p>';
 		        echo '<label for="' . esc_attr( $this -> get_field_id( 'title' ) ) . '">' . __( 'Title', 'gardener' );
