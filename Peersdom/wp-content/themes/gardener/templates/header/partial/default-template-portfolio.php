<?php
    ...
    if( has_post_thumbnail( $post -> ID ) && isset( $thumbnail -> ID ) || tempo_has_header() ){
    	...
    	$media = wp_get_attachment_image_src( $thumbnail -> ID, apply_filters( 'gardener_header_thumbnail_size', 'gardener-header' ) );
...
?>
		<div class="tempo-header-partial tempo-portfolio" style="background-image: url(<?php echo esc_url( $image ); ?>);">

			<?php tempo_get_template_part( 'templates/header/partial/prepend' ); ?>

			<!-- mask - a transparent foil over the header image -->
    		<div class="tempo-header-mask" style="<?php echo esc_attr( apply_filters( 'gardener_single_mask_background', null, $post ) ); ?>"></div>
